...
def populate_summary(
    ...
    if not transpose:
        populate_summary_horizontal(ws, source_last_row, use_thousands, decimal_mode, aggregates)
        return
    ...
    for row_idx in range(2, ws.max_row + 1):
        ...
        for col_idx in range(2, ws.max_column + 1):
            ...
            if metric in percent_metrics:
                cell.number_format = percent_format(decimal_mode)
...
def populate_summary_horizontal(ws, source_last_row, use_thousands=True, decimal_mode="fixed2", aggregates=None):
